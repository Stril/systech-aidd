...
- [ ] Создать тесты для `MessageExtractor`
- [ ] Создать тесты для `BotMessages`
- [ ] Обновить существующие тесты `MessageHandler` под новую структуру
- [ ] Запустить `make test` - все тесты должны пройти
- [ ] Запустить `make quality` - проверка качества кода
